...
# Enable test mode if TEST_MODE is set to "true"
TEST_MODE = os.environ.get('TEST_MODE', 'false').lower() == 'true'
logging.info("TEST_MODE: %s", TEST_MODE)

# Remove global youtube client. Instead, each /process call will build one using the user's API key.
...
def get_video_details(client, video_ids):
    """Fetch metadata (view count, duration) for video IDs in chunks (or return dummy details in test mode)."""
    details = {}
